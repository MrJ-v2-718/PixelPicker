...
def view_help():
    messagebox.showinfo(
        ...
        "Clicking on a pixel gives you the co-ordinates and RGB values. "
        "Happy picking."
    )
...
def show_about():
    messagebox.showinfo(
        "About",
        "PixelPicker - A Simple Image Viewer\n"
        f"\tCreated by MrJ\n\t         2024"
    )
...
class PixelPicker(tk.Tk):
    ...
    def on_mouse_wheel(self, event):
        # Check the direction of the mouse wheel scroll and adjust the scrollbar
        if event.delta > 0:
